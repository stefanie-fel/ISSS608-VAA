...
-   There are couple of striking message mentioned in the exhanges e.g. "keep tourism appearance", "keep operations discreet" or "we need to stay under radar", which sounds sounds suspicious
:::
...
timeline_cr <- create_tml(df=timeline_nadia_comm, smr="timestamp_desc", dsc="content", smr_bgcol = "#2E8BC0", dsc_col = "#2E8BC0")
timeline_cr
```

```{r}
comm_encryption <- mc3_graph %>% activate(nodes) %>% filter(str_detect(content, "Delta-3 encryption"))
