...
```{r}
pacman::p_load(tidyverse, jsonlite, SmartEDA, tidygraph, ggraph, knitr, DT, stringr)
```
...
comm_about_nadia <- mc3_graph %>%
  activate(nodes) %>%
  filter(str_detect(content, "Nadia Conti"))


comms_table <- as_tibble(comm_about_nadia, active = "nodes")
datatable(comms_table)
```
