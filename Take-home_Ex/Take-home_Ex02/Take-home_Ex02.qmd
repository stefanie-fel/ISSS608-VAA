...
```{r}
pacman::p_load(tidyverse, jsonlite, SmartEDA, tidygraph, ggraph, knitr, DT)
```
...
entity_table \<- as_tibble(graph_unqiue_person, active = "nodes")

graph_memberof \<- graph %\>% activate(edges) %\>% filter(`Edge Type` == "Memberof")
