...
```{r}
pacman::p_load(tidyverse, jsonlite, SmartEDA, tidygraph, ggraph, DT)
```
...
table_data <- matrix(
  c("jsonlite",      "Convert JSON data to R objects",
    ...
    "kable", "Creates simple table"),
  nrow = 7,
  byrow = TRUE
)

df <- as.data.frame(table_data, stringsAsFactors = FALSE)
colnames(df) <- c("Package Name", "Description")

datatable(df, colnames = c("Package Name", "Description"))
```
