...
timeline_nadia_comm <- timeline_nadia_comm %>%
  ...
  timestamp = as.POSIXct(timestamp, format = "%Y-%m-%d %H:%M:%S"),
    timestamp_desc = format(timestamp, "%A, %B %d, %Y"))

timeline_nadia <- create_tml_2(df=timeline_nadia_comm, smr="timestamp_desc", dsc="label", dsc2="content")
timeline_nadia
```
...
comm_about_permit %>% as_tibble() %>% select(label, id, timestamp, content,monitoring_type, findings, new_index) %>% datatable()
```
...
colnames(timeline_nadia_comm)
colnames(comm_about_nadia)

timeline_nadia <- create_tml_2(df=timeline_nadia_comm, smr="timestamp", dsc="label", dsc2="content")
