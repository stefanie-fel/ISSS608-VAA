...
```{r}
pacman::p_load(tidyverse, jsonlite, SmartEDA, tidygraph, ggraph, knitr, DT, stringr, cronologia, igraph)
```
...
-   People that are involved: Rodriquez, Mako, Davis, Haackle Harbor, Oceanus City Council, Neptune, Elise, Marlin, Liam, V. Miesel Shipping, V. Miesel HQ, Glitter's Team, The Middleman and Ramora
...
```{r}
comm_13_oct <- mc3_graph %>% activate(nodes) %>% filter(str_detect(timestamp, "2040-10-14"))

comm_13_oct <- comm_13_oct %>%
  as_tibble(active = "nodes") %>%  
  select(label, id, timestamp, content, monitoring_type, findings, new_index) %>% datatable()
comm_13_oct
```

```{r}
nadia_relationship <- mc3_graph %>%
  activate(nodes) %>%
