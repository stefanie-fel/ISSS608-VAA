...
When using ggplot2, we can customize how the graph looks like as ggplot2 offers in-built themes (e.g. $theme_gray$, $theme_bw$, $theme_classic$, $theme_dark$$, theme_light$, $theme_linedraw$, $theme_minimal$ and $theme_void$). In this example, $theme_gray$ is used, which looks like this:
...
hrbthems package provides a base theme that focus on typographic elemenyts (incl. where labels are placed and fonts)
