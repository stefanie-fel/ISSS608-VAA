...
```{r}
p <- ggplot(data=df, aes(x=date, y=close)) + geom_line(color="steelblue") + geom_hline(yintercept=mean_close, color="grey0", linetype="dashed", size="0.5, alpha=0.8") + 
  ...
  scale_x_date(date_breaks = "1 month", "date_labels" = "%b %Y") + labs(title=dynamic_title, y = "Closing Price", x ="") +
  theme_classic()
